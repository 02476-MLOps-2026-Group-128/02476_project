...
Our API exposes a monitoring endpoint (`/monitoring`), which provides reports generated by Evidently about the data drift, data quality and target drift.

The monitoring endpoint makes use of FastAPI's `BackgroundTasks` on each request to the `predict` endpoint to store the incoming data, making sure that the request is not delayed by the storage operation.
