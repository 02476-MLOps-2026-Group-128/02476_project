...
def build_payload(
    ...
    Returns:
        Feature payload with all expected keys. The returned dictionary has two keys: "raw" and "payload".
    ...
    for feature in feature_list:
        if feature in numeric_stats:
            ...
            raw_payload[feature] = float(raw_value)
            if normalize_inputs:
            ...
            raw_payload[feature] = float(categorical_values[feature])
            payload[feature] = float(categorical_values[feature])
        else:
            raw_payload[feature] = float(categorical_defaults.get(feature, 0.0))
...
def post_prediction(backend_url: str, payload: dict[str, dict[str, float]]) -> tuple[dict[str, Any] | None, str | None]:
    """
    Send the payload to the prediction API.

    Args:
        backend_url: Base URL of the backend service.
        payload: Feature payload, containing both raw and normalized features.
